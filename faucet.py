...
class Faucet(commands.Cog):
    ...
    async def register(self,ctx, arg1=None):
        if arg1 is None:
            await ctx.channel.send("You need to register a "+config.CRYPTO_TICKER+" address")
            return

        rpc_connection_CRYPTO = 'http://{0}:{1}@{2}:{3}'.format(config.CRYPTO_RPC_USER, config.CRYPTO_RPC_PW, config.CRYPTO_RPC_IP, config.CRYPTO_RPC_PORT)
        ...
        if user_captcha_response is False:
            ...
            if str(answer) == str(user_captcha_response.replace(" ","").replace("\n","")):
                async with ctx.author.typing():
                    ...
                    if amount is None:
                        if(current_balance < config.FAUCET_MIN_WITHDRAW):
                        ...
                        rpc_connection_CRYPTO = 'http://{0}:{1}@{2}:{3}'.format(config.CRYPTO_RPC_USER, config.CRYPTO_RPC_PW, config.CRYPTO_RPC_IP, config.CRYPTO_RPC_PORT)
                        wallet = AuthServiceProxy(rpc_connection_CRYPTO)
                        txid = wallet.sendfrom(config.FAUCET_SOURCE,to_address, current_balance)
                        ...
                        amount=float(amount)
                        if(amount < config.FAUCET_MIN_WITHDRAW):
                            print("you need to be above minimum withdraw balance")
                            return
                        ...
                        rpc_connection_CRYPTO = 'http://{0}:{1}@{2}:{3}'.format(config.CRYPTO_RPC_USER, config.CRYPTO_RPC_PW, config.CRYPTO_RPC_IP, config.CRYPTO_RPC_PORT)
                        wallet = AuthServiceProxy(rpc_connection_CRYPTO)
                        txid = wallet.sendfrom(config.FAUCET_SOURCE,to_address, amount)
                        await confirmation_message.edit(content=txid)
    ...
    async def claim(self,ctx):
        ...
        if current_time - last_claimed_time <= config.FAUCET_TIME_DELAY:
        ...
        if user_captcha_response == False:
            ...
            modify_db_balance(ctx.author.id,config.FAUCET_REWARD)
